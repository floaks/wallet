...
var UI = (function(UI, undefined) {
  ...
  UI.updateStatusBar = function(data) {
    ...
    if (data.hasOwnProperty("hoverAmount")) {
      if (data.hoverAmount === false) {
        document.getElementById("status-bar-gflkd").style.display = "none";
      } else {
        document.getElementById("status-bar-gflkd").style.display = "inline";
        document.getElementById("status-bar-gflkd").innerHTML = UI.convertToMflkds(data.hoverAmount);
      }
    }
  }

  UI.convertToMflkds = function(amount) {
  ...
  UI.showKillAlert = function() {
    ...
    modal.setContent("<h1 data-i18n='shutdown_in_progress'>" + UI.t("shutdown_in_progress") + "</h1><p style='margin-bottom:0' data-i18n='shutting_down_flkd'>" + UI.t("shutting_down_flkd") + "</p>");
  ...
  UI.handleURL = function(url) {
    ...
    url = decodeURI(url.replace("flkd://", "").toLowerCase().replace(/\/$/, ""));
