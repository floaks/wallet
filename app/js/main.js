...
 
    if (process.platform == "darwin") {
      ...
      if (process.execPath.match(/\/Volumes\/flkd Wallet/i)) {
  ...
  App.registerProtocol = function() {
    ...
    if (!electron.app.isDefaultProtocolClient("flkd")) {
      console.log("Register flkd as a default protocol");
      electron.app.setAsDefaultProtocolClient("flkd"); //not linux
    }
  }
  ...
  App.autoUpdate = function() {
    ...
    if (process.platform == "darwin") {
      var feedURL = "https://flkd.org/latest-osx.php?v=" + appVersion;
    } else {
      var feedURL = "https://flkd.org/latest-win.php?v=" + appVersion + "&arch=" + (is64BitOS ? "64" : "32");
    }
    ...
    template.push(
    {
      ...
      submenu: [
        ...
        },
        {
          ...
          click() { shell.openExternal("https://flkd.org/"); }
        },
        {
          label: App.t("forum"),
          click() { shell.openExternal("https://forum.flkd.org/"); }
        },
        {
          label: App.t("chat"),
          click() { shell.openExternal("https://slack.flkd.org/"); }
        },
        {
          label: App.t("documentation"),
          click() { shell.openExternal("https://flkd.readme.io/docs"); }
        },
        {
          label: App.t("submit_bug_report"),
          click() { shell.openExternal("https://github.com/floaks/wallet/issues"); }
        }
      ]
    });
  ...
  App.loadEnvironment = function() {
    ...
    if (process.platform == "win32" && process.env.LOCALAPPDATA) {
      var oldAppDataDirectory = path.join(electron.app.getPath("appData"), "flkd Wallet" + (isTestNet ? " Testnet" : ""));

      electron.app.setPath("appData", process.env.LOCALAPPDATA);
      electron.app.setPath("userData", path.join(process.env.LOCALAPPDATA, "flkd Wallet" + (isTestNet ? " Testnet" : "")));

      var newAppDataDirectory = path.join(electron.app.getPath("appData"), "flkd Wallet" + (isTestNet ? " Testnet" : ""));
      ...
      appDataDirectory = path.join(electron.app.getPath("appData"), "flkd Wallet" + (isTestNet ? " Testnet" : ""));
    }
      ...
      server.on("exit", function(code) {
        ...
        // System is not closing automatically, wait for user to click the alert button.
          ...
          if (!isStarted) {
            ...
            App.showAlertAndQuit("server_exited", "flkd_server_process_exited");
            return;
          }
        } else if (!doNotQuit) {
    ...
    if (!isStarted && !didKillNode && !nodeInitializationError)   {
      if (type == "error") {
        if (data.match(/java\.net\.BindException/i)) {
          ...
          var error = data.match(/ERROR\s*com\.flkd\.iri\.IRI\s*\-\s*(.*)/i);
        ...
        if (data.match(/flkd Node initialised correctly/i)) {
          App.nodeStarted();
        }
      }
    } else if (type == "error") {
  ...
  App.editNodeConfiguration = function(walletType) {
    if (App.windowIsReady()) {
      ...
      if (walletType == 1) {
        ...
        var urls = [
          'https://flkdsupport.com/providers.json',
          'https://static.flkd.org/providers.json'
        ]
  ...
  App.updateTitle = function(includeNodeType, _isTestNet) {
    ...
    var title = "flkd " + (includeNodeType && settings.lightWallet == 1 ? "Light " : "") + "Wallet " + App.format(appVersion.replace("-testnet", "")) + (_isTestNet ? " - Testnet" : "") + (iriVersion ? " - IRI " + App.format(iriVersion) : "");
...
const shouldQuit = electron.app.makeSingleInstance(function(commandLine, workingDirectory) {
  ...
  if (win) {
    ...
    if (process.platform == "win32" && commandLine.length == 2) {
      if (String(commandLine[1]).match(/^flkd:\/\//i)) {
