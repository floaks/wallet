var UI = (function(UI, $, undefined) {
  UI.handleHistory = function() {
    ...
    $("#history-stack").on("click", ".show-bundle", function(e) {
      ...
      flkd.api.getBundle(hash, function(error, transactions) {
        if (error) {
          return;
        }
        ...
        flkd.api.isReattachable(inputAddresses, function(error, isReattachable) {
          //TEMP BUG FIX
          ...
          for (var i=0; i<transactions.length; i++) {
            ...
            html += "<li><div class='details'><div class='address'>" + (tag ? "<div class='tag'>" + UI.format(tag) + "</div>" : "") + UI.formatForClipboard(flkd.utils.addChecksum(transactions[i].address)) + "</div></div><div class='value'>" + UI.formatAmount(transactions[i].value) + "</div></li>";
          }
    ...
    $("#reattach-btn, #rebroadcast-btn").on("click", function(e) {
      ...
      if (isRebroadcast) {
        UI.isDoingPOW = true;
        flkd.api.broadcastBundle(hash, function(error, bundle) {
          UI.isDoingPOW = false;
          if (error) {
        ...
        UI.isDoingPOW = true;
        flkd.api.replayBundle(hash, connection.depth, connection.minWeightMagnitude, function(error, bundle) {
          UI.isDoingPOW = false;
          if (error) {
  ...
  UI.updateHistory = function() {
    ...
    if (connection.accountData) {
      var addresses = flkd.utils.addChecksum(connection.accountData.addresses).reverse();

      var categorizedTransfers = flkd.utils.categorizeTransfers(connection.accountData.transfers, connection.accountData.addresses);
      ...
      $.each(connection.accountData.transfers.reverse(), function(i, bundle) {
        ...
        transfersHtml += "<div class='address'>" + (address ? UI.formatForClipboard(flkd.utils.addChecksum(address)) : "/") + "</div>";
    ...
    $.each(addresses, function(i, address) {
      ...
      addressesHtml += "<div class='address'" + (spentAddresses.indexOf(flkd.utils.noChecksum(address)) != -1 ? " style='text-decoration:line-through'" : "") + ">" + UI.formatForClipboard(address) + "</div>";
      addressesHtml += "</div>";
