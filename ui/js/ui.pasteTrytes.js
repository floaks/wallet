    ...
    if ($.isArray(bundle)) {
      $.each(bundle, function(index, trytes) {
        if (!flkd.valid.isTrytes(trytes)) {
          ...
          var transaction = flkd.utils.transactionObject(trytes);
          if (transaction) {
  ...
  UI.handlePastingTrytes = function() {
    $(document).on("closed", "#paste-trytes-modal", function (e) {
      if (isProcessing) {
        ...
        flkd.api.interruptAttachingToTangle();
      }
    });
    ...
    $("#verify-pasted-trytes-btn").on("click", function(e) {
      ...
      if (!bundleTxs) {
        ...
        return;
      } else if (!flkd.utils.isBundle(bundleTxs)) {
      ...
      for (var i=0; i<bundleTxs.length; i++) {
        html += "<li><div class='details'><div class='address'>" + UI.formatForClipboard(flkd.utils.addChecksum(bundleTxs[i].address)) + "</div></div><div class='value'>" + UI.formatAmount(bundleTxs[i].value) + "</div></li>";
      }
    ...
    $("#process-pasted-trytes-btn").on("click", function(e) {
      ...
      $.each(bundleTxs, function(index, transaction) {
        trytes.push(flkd.utils.transactionTrytes(transaction));
      });
      ...
      UI.isDoingPOW = true;
      flkd.api.sendTrytes(trytes, connection.depth, connection.minWeightMagnitude, function(error, transfers) {
        UI.isDoingPOW = false;
        if (error) {
