var UI = (function(UI, $, undefined) {
  ...
  UI.onOpenAddressStack = function() {
    ...
    var latestAddress = flkd.utils.addChecksum(connection.accountData.latestAddress);
  ...
  UI.handleAddressGeneration = function() {
    $("#generate-address-btn").on("click", function(e) {
      ...
      try {
        ...
        flkd.api.getNewAddress(connection.seed, {"checksum": true}, function(error, newAddress) {
          $loader.fadeOut();
          ...
          newAddress = flkd.utils.noChecksum(newAddress);

          UI.animateStacks(200);
            
          flkd.api.sendTransfer(connection.seed, connection.depth, connection.minWeightMagnitude, [{"address": newAddress, "value": 0, "message": "", "tag": ""}], function(error, transfers) {
            UI.isDoingPOW = false;
            if (error) {
              UI.formError("generate-address", error);
            } else {
