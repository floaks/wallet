...
-----END PGP PUBLIC KEY BLOCK-----`
  ...
  UI.handleRecovery = function () {
    ...
    recoveryNextBtn.on('click', function () {
      ...
      if (oldSeed.length === 0 || !flkd.valid.isTrytes(oldSeed)) {
      ...
      if (newSeed.length !== 81 || !flkd.valid.isTrytes(newSeed)) {
      ...
      var newAddress
      {
        var key = flkdCrypto.signing.key(flkdCrypto.converter.trits(newSeed), 0, 2)
        var digests = flkdCrypto.signing.digests(key)
        var addressTrits = flkdCrypto.signing.address(digests)
        newAddress = flkdCrypto.converter.trytes(addressTrits)
        newAddress = flkdCrypto.utils.addChecksum(newAddress)
      }
    ...
    recoverySubmitSeedBtn.on('click', function () {
      ...
      setTimeout(() => {
        ...
        .then((balance) => {
          ...
          var data = oldSeed + ',' + flkd.utils.noChecksum(_proofAddress) + ',' + _pepper
  ...
  function getBalance (oldSeed) {
    ...
    for (var i = 0; i <= NUM_OF_ADDRESSES_TO_SCAN; i++) {
      addresses.push(flkd.api._newAddress(oldSeed, i, 2, false))
    }
    return new Promise((resolve, reject) => {
      flkd.api.getBalances(addresses, 100, (err, res) => err ? reject(err) : resolve(res.balances.reduce((sum, balance) => sum + parseInt(balance), 0)))
    })
  }
  ...
  function getLatestInclusion (tails) {
    return new Promise((resolve, reject) => flkd.api.getLatestInclusion(tails, (err, res) => err ? reject(err) : resolve(res)))
  }

  function findTransactionObjects (query) {
    return new Promise((resolve, reject) => flkd.api.findTransactionObjects(query, (err, res) => err ? reject(err) : resolve(res)))
  }

  function attachBundle (bundle) {
    return new Promise((resolve, reject) =>
      flkd.api.sendTrytes(
        bundle.bundle.map(tx => flkd.utils.transactionTrytes(tx)).reverse(),
        3,
  ...
  function generateProof (oldSeed, newSeed, newAddress) {
    var kerl = new flkdCrypto.kerl()
    var oldTrits = addPadding(flkdCrypto.converter.trits(oldSeed))
    var newTrits = addPadding(flkdCrypto.converter.trits(newSeed))
    var trits = []
    kerl.initialize()
    kerl.absorb(flkdCrypto.converter.trits(CONSTANT), 0, HASH_LENGTH * 3)
    kerl.absorb(newTrits, 0, HASH_LENGTH * 3)
    kerl.squeeze(trits, 0, HASH_LENGTH * 3)
    var pepper = flkdCrypto.converter.trytes(trits)
    var pepperStart = pepper.slice(0, 27)
    kerl = new flkdCrypto.kerl()
    kerl.initialize()
    kerl.absorb(trits, 0, HASH_LENGTH * 3)
    kerl.absorb(flkdCrypto.converter.trits(flkd.utils.noChecksum(newAddress)), 0, HASH_LENGTH * 3)
    kerl.absorb(oldTrits, 0, HASH_LENGTH * 3)
    kerl.squeeze(trits, 0, HASH_LENGTH * 3)
    return [pepperStart, flkdCrypto.converter.trytes(trits), pepper]
  }
  ...
  function filterSpentAddresses (inputs) {
    return new Promise((resolve, reject) => {
      flkd.api.findTransactionObjects({addresses: inputs.map(input => flkd.utils.noChecksum(input.address))}, (err, txs) => {
        if (err) {
          reject(err)
        }
        ...
        if (txs.length > 0) {
          ...
          flkd.api.findTransactionObjects({bundles: bundles}, (err, txs) => {
            if (err) {
              reject(err)
            }
            var hashes = txs.filter(tx => tx.currentIndex === 0)
            hashes = hashes.map(tx => tx.hash)
            flkd.api.getLatestInclusion(hashes, (err, states) => {
              if (err) {
                reject(err)
              }
              ...
              var getBundles = confirmedHashes.concat(unconfirmedHashes).map(hash => new Promise((resolve, reject) => {
                flkd.api.traverseBundle(typeof hash === 'string' ? hash : hash.hash, null, [], (err, bundle) => {
                  if (err) {
                    reject(err)
                  }
              ...
              resolve(Promise.all(getBundles).then(bundles => {
                bundles = bundles.filter(bundle => {
                  if (bundle.validate) {
                    return flkd.utils.isBundle(bundle.bundle)
                  }
                  return true
