  ...
  UI.handleTransfers = function() {
    ...
    $("#transfer-btn").on("click", function(e) {
      ...
      try {
        ...
        } else if (!flkd.utils.isValidChecksum(address)) {
        ...
        if (!rawAmount) {
          ...
          amount = flkd.utils.convertUnits(parseFloat(rawAmount), rawUnits, "i");
      ...
      getUnspentInputs(connection.seed, 0, amount, function(error, inputs) {
        ...
        var outputsToCheck = transfers.map(transfer => { return {address: flkd.utils.noChecksum(transfer.address)}});
        ...
        filterSpentAddresses(outputsToCheck).then(filtered => {
          ...
          flkd.api.prepareTransfers(connection.seed, transfers, {"inputs": inputs.inputs}, function(error, trytes) {
            if (error) {
              UI.formError("transfer", error, {"initial": "send_it_now"});
              $stack.removeClass("loading");
              return;
            ...
            trytes.forEach(transactionTrytes => {
              var tx = flkd.utils.transactionObject(transactionTrytes);
            ...
            flkd.api.sendTrytes(trytes, connection.depth, connection.minWeightMagnitude, (error, transfers) => {
              UI.isDoingPOW = false;
              if (error) {
                UI.formError("transfer", error, {"initial": "send_it_now"});
              } else {
    ...
    $("#transfer-units-value").on("click", function(e) {
      ...
      var $select = $('<div class="dropdown" id="transfer-units-select">' +
                        '<ul>' +
                          '<li class="flkd-i">i</li>' +
                          '<li class="flkd-ki">Ki</li>' +
                          '<li class="flkd-mi">Mi</li>' +
                          '<li class="flkd-gi">Gi</li>' +
                          '<li class="flkd-ti">Ti</li>' +
                        '</ul>' +
                      '</div>');
...
function filterSpentAddresses(inputs) {
  return new Promise((resolve, reject) => {
    flkd.api.findTransactionObjects({addresses: inputs.map(input => input.address)}, (err, txs) => {
      if (err) {
        reject(err)
      }
...
function getUnspentInputs(seed, start, threshold, inputs, cb) {
  ...
  flkd.api.getInputs(seed, {start: start, threshold: threshold}, (err, res) => {
    if (err) {
      cb(err, inputs)
      return
    }
